      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex gap-8 animate-pulse">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="w-32 h-16 bg-slate-200 rounded-lg"></div>
            ))}
          </div>
        </div>
      </section>
              ...
              {sponsors.map((sponsor) => (
                <div key={sponsor.id} className="group">
                  <a
                    ...
                    className="block p-4 rounded-xl bg-white shadow-sm hover:shadow-lg transition-all duration-300 group-hover:scale-105"
                  >
                    <img
