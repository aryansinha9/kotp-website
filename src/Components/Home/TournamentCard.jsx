// REPLACE THE CONTENTS OF: src/Components/home/TournamentCard.jsx
  ...
  return (
    <Card className="card-hover border-0 shadow-xl overflow-hidden bg-white/70 backdrop-blur-sm">
      <div className="relative">
        <img src={tournament.hero_image || "https://images.unsplash.com/photo-1574629810360-7efbbe195018?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80"} alt={tournament.name} className="w-full h-48 object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent"></div>
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          {isEarlyBird && (<Badge className="bg-amber-500 text-white border-amber-400 shadow-lg"><Clock className="w-3 h-3 mr-1" />Early Bird</Badge>)}
          {isAlmostFull && (<Badge className="bg-red-500 text-white border-red-400 shadow-lg animate-pulse"><AlertCircle className="w-3 h-3 mr-1" />Almost Full</Badge>)}
        </div>
      ...
      <CardContent className="pt-0">
        <div className="space-y-4 mb-6">
          <div className="flex items-center gap-3 text-slate-600">
            ...
            <span className="font-medium">{format(new Date(tournament.start_date), "MMM d")} - {format(new Date(tournament.end_date), "MMM d, yyyy")}</span>
          </div>
          <div className="flex items-center gap-3 text-slate-600">
          ...
          <div className="flex items-center gap-3 text-slate-600">
            ...
            <span className="font-medium">{tournament.age_groups?.join(", ") || "Multiple age groups"}</span>
          </div>
        ...
        <div className="flex gap-3">
          ...
          <Link to={`/tournament-details?id=${tournament.id}`}>
            <Button variant="outline" className="border-slate-300 text-slate-700 hover:bg-slate-100">Details</Button>
          </Link>
        </div>
      </CardContent>
    </Card>
