// src/Pages/AdminDashboard.jsx (FINAL, FULLY FUNCTIONAL, AND COMPLETE)
...
const Section = ({ icon: Icon, title, count, defaultOpen = false, children }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#1a1a1a] border border-white/10 rounded-lg overflow-hidden">
      ...
      <AnimatePresence>
        {isOpen && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden"><div className="p-6 border-t border-white/10">{children}</div></motion.div>)}
      </AnimatePresence>
    </motion.div>
  );
};
...
const StyledButton = ({ children, onClick, disabled }) => <Button onClick={onClick} disabled={disabled} className="kotp-button bg-[#FF6B00] text-white px-8 py-3 rounded-md headline-font tracking-wider hover:bg-[#FF6B00]/90 transition-all duration-300 flex items-center justify-center w-full md:w-auto">{children}</Button>;
const StyledTextarea = ({ value, onChange, placeholder }) => <Textarea value={value} onChange={onChange} placeholder={placeholder} className="bg-[#0a0a0a] border-white/10 text-white focus:border-[#FF6B00] min-h-[120px] font-mono text-sm" />;
...
export default function AdminDashboard() {
  ...
  const handleGallerySubmit = async () => {
    ...
    else { setUploadMessage({ type: "success", text: "All files uploaded successfully!" }); setSelectedFiles([]); if (document.getElementById('file-upload')) document.getElementById('file-upload').value = ""; setTimeout(() => setUploadMessage({ type: "", text: "" }), 5000); }
    setUploading(false);
  };
  ...
  const handleStatusChange = async (gameId, currentStatus) => {
    ...
    if (updatedGame) {
      const updatedGames = games.map(g => g.id === gameId ? updatedGame : g).sort((a, b) => new Date(a.start_time) - new Date(b.start_time));
              ...
              <div>
                ...
                <Select value={selectedScoreTournamentId} onValueChange={setSelectedScoreTournamentId}>
                  ...
                  <SelectContent className="bg-[#1a1a1a] border-white/10">
                    {ongoingTournaments.length === 0 ? <div className="p-4 text-gray-500 text-center">No ongoing tournaments</div> : ongoingTournaments.map((t) => (<SelectItem key={t.id} value={String(t.id)} className="text-white hover:bg-white/5">{t.name}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
                ...
                <>
                  <div className="bg-[#0a0a0a] border border-white/10 rounded-lg p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <Button onClick={handleCreateGame} disabled={isCreatingGame} className="kotp-button bg-[#FF6B00] text-white headline-font h-12">{isCreatingGame ? "CREATING..." : "CREATE GAME"}</Button>
                    </div>
                  </div>

                  {liveGames.length > 0 && (<div><h3 className="headline-font text-2xl text-white mb-4 flex items-center gap-2"><span className="w-3 h-3 bg-red-500 rounded-full animate-pulse"></span>LIVE ({liveGames.length})</h3><div className="space-y-4">{liveGames.map((game) => (<div key={game.id} className="bg-[#0a0a0a] border-2 border-red-500/30 rounded-lg p-6"><div className="flex items-center justify-between mb-4"><span className="bg-red-500 text-white px-3 py-1 rounded-full text-xs headline-font">LIVE</span><Button onClick={() => handleStatusChange(game.id, game.status)} className="bg-[#1a1a1a] border border-white/10 text-white hover:bg-white/5 headline-font">END GAME</Button></div><div className="grid grid-cols-3 gap-4 items-center"><div className="text-right"><p className="headline-font text-xl text-white mb-2">{getTeamName(game.team_a_id)}</p><div className="flex items-center justify-end gap-2"><Button onClick={() => handleScoreChange(game.id, "team_a_score", false)} size="icon" variant="outline" className="border-white/10 text-white h-10 w-10"><Minus className="w-4 h-4" /></Button><span className="text-4xl headline-font text-[#FF6B00] w-12 text-center">{game.team_a_score || 0}</span><Button onClick={() => handleScoreChange(game.id, "team_a_score", true)} size="icon" className="bg-[#FF6B00] text-white h-10 w-10"><Plus className="w-4 h-4" /></Button></div></div><div className="text-center"><span className="text-3xl text-gray-500">VS</span></div><div className="text-left"><p className="headline-font text-xl text-white mb-2">{getTeamName(game.team_b_id)}</p><div className="flex items-center gap-2"><Button onClick={() => handleScoreChange(game.id, "team_b_score", false)} size="icon" variant="outline" className="border-white/10 text-white h-10 w-10"><Minus className="w-4 h-4" /></Button><span className="text-4xl headline-font text-[#FF6B00] w-12 text-center">{game.team_b_score || 0}</span><Button onClick={() => handleScoreChange(game.id, "team_b_score", true)} size="icon" className="bg-[#FF6B00] text-white h-10 w-10"><Plus className="w-4 h-4" /></Button></div></div></div></div>))}</div></div>)}
                  {scheduledGames.length > 0 && (<div><h3 className="headline-font text-2xl text-white mb-4">SCHEDULED ({scheduledGames.length})</h3><div className="space-y-4">{scheduledGames.map((game) => (<div key={game.id} className="bg-[#0a0a0a] border border-white/10 rounded-lg p-6"><div className="flex items-center justify-between"><div className="flex-1 grid grid-cols-3 gap-4 items-center"><div className="text-right"><p className="headline-font text-lg text-white">{getTeamName(game.team_a_id)}</p></div><div className="text-center"><span className="text-xl text-gray-500">VS</span></div><div className="text-left"><p className="headline-font text-lg text-white">{getTeamName(game.team_b_id)}</p></div></div><Button onClick={() => handleStatusChange(game.id, game.status)} className="bg-[#FF6B00] text-white headline-font ml-4">START GAME</Button></div></div>))}</div></div>)}
                  {finalGames.length > 0 && (<div><h3 className="headline-font text-2xl text-white mb-4">FINAL SCORES ({finalGames.length})</h3><div className="space-y-4">{finalGames.map((game) => (<div key={game.id} className="bg-[#0a0a0a] border border-white/10 rounded-lg p-6 opacity-60"><div className="flex items-center justify-between mb-2"><span className="bg-gray-500 text-white px-3 py-1 rounded-full text-xs headline-font">FINAL</span></div><div className="grid grid-cols-3 gap-4 items-center"><div className="text-right"><p className="headline-font text-lg text-white">{getTeamName(game.team_a_id)}</p><span className="text-3xl headline-font text-white">{game.team_a_score || 0}</span></div><div className="text-center"><span className="text-xl text-gray-500">VS</span></div><div className="text-left"><p className="headline-font text-lg text-white">{getTeamName(game.team_b_id)}</p><span className="text-3xl headline-font text-white">{game.team_b_score || 0}</span></div></div></div>))}</div></div>)}
                </>
              )}
            </div>
          </Section>
