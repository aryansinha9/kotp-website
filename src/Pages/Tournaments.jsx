// src/Pages/Tournaments.jsx (Corrected - Duplicate component removed)
  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-grow">
        <h3 className="headline-font text-2xl text-white mb-4 group-hover:text-[#FF6B00] transition-colors">{cardData.name}</h3>
        ...
        <div className="flex items-center justify-between">
          ...
          <Link to={`/register/${tournament.id}`}>
            <button className="kotp-button bg-[#FF6B00] text-white px-6 py-2 rounded-md headline-font text-sm tracking-wider hover:scale-105 transition-transform duration-300">REGISTER</button>
          </Link>
        </div>
      </div>
    </motion.div>
      ...
      <section className="py-24 px-4 bg-[#0a0a0a]">
        <div className="max-w-4xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            ...
            <Link to="/tournaments">
              <button className="kotp-button bg-[#FF6B00] text-white px-12 py-4 rounded-md headline-font text-xl tracking-wider pulse-glow hover:scale-105 transition-transform duration-300">REGISTER FOR NEXT TOURNAMENT</button>
            </Link>
          </motion.div>
        </div>
      </section>
